...
public class GraphOperationsPart3 {

    public static Set<Vertex> getNeighbours(Graph graph, Vertex vertex) {
        return graph.getEdges().stream()
                .filter(v -> v.getV1().equals(vertex) || v.getV2().equals(vertex))
                .map(v -> {
                    if (!v.getV1().equals(vertex)) {
                        return v.getV1();
                    } else {
                        return v.getV2();
                    }
    ...
    public static int departmentConnections(Graph graph, Set<Vertex> department1, Set<Vertex> department2) {
        ...
        List<Vertex> neighboursDepartment1 = department1.stream()
                .flatMap(d -> getNeighbours(graph, d).stream())
                .collect(Collectors.toList());

            return (int) neighboursDepartment1.stream()
                    .filter(department2::contains)
                    .count();
    }
